...
async function crear(req, res, next) {
  try {
    ...
    const { producto_id, titulo, descripcion, cantidad_disponible, precio_unitario, unidad_medida, municipio, departamento, fecha_cosecha, imagen_url } = req.body;
    if (!producto_id || !titulo || !cantidad_disponible || !precio_unitario || !unidad_medida)
      return res.status(400).json({ success:false, message:'Faltan campos requeridos' });
    const data = await prisma.publicacion.create({ data:{ productor_id:productor.id, producto_id:Number(producto_id), titulo, descripcion, cantidad_disponible:Number(cantidad_disponible), precio_unitario:Number(precio_unitario), unidad_medida, municipio, departamento, fecha_cosecha: fecha_cosecha ? new Date(fecha_cosecha) : null, imagen_url } });
